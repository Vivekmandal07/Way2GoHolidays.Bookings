...
const LoginModal: React.FC<LoginModalProps> = ({ onClose, onLoginSuccess }) => {
  ...
  const DEFAULT_USERS: AuthUser[] = [
    {
      ...
      password: 'Admin@123',
      role: 'admin',
      identifier: 'admin@way2go.com'
    },
    {
      id: 'staff-1',
      name: 'Staff User',
      email: 'staff@way2go.com',
      password: 'Staff@123',
      role: 'staff',
      identifier: 'staff@way2go.com'
    }
  ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isRegister) {
      if (!formData.password.trim()) {
        setLoginError('Please set a secure password.');
        return;
      }

      const existing = [...DEFAULT_USERS, ...users].find(user => user.identifier === contactIdentifier);
      if (existing) {
        setLoginError('This account already exists. Please log in.');
        return;
      }
      ...
      const newUser: AuthUser = {
        ...
        email: method === 'email' ? contactIdentifier : undefined,
        phone: method === 'phone' ? contactIdentifier : undefined,
  ...
  const sendOtp = () => {
    ...
    if (!phone) {
      setLoginError('Enter valid mobile number to receive OTP.');
      return;
    }
    setOtpSent(true);
    setLoginError('OTP sent to your mobile number. Use 123456 to login.');
  };
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="relative bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-500">
        ...
        <div className="flex p-2 bg-slate-50 border-b border-slate-100">
          <button 
            onClick={() => { setMethod('email'); setPhoneMode('password'); setLoginError(''); }}
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          <div className="space-y-6">
            ...
            <div className="space-y-2 relative">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">
                {method === 'phone' && !isRegister && phoneMode === 'otp' ? 'Enter OTP' : 'Secure Password'}
              </label>
              <div className="relative">
                {method === 'phone' && !isRegister && phoneMode === 'otp' ? (
                  <input 
                    required
                    type="text"
                    className="w-full px-6 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-blue-500 focus:bg-white transition-all font-bold text-slate-900 placeholder:text-slate-300 shadow-sm"
                ...
                {method === 'phone' && !isRegister && (
                  <div className="flex items-center justify-between mt-2 text-xs font-bold text-slate-500">
                    <button
                      type="button"
                      onClick={() => {
                        setPhoneMode(phoneMode === 'password' ? 'otp' : 'password');
                        setLoginError('');
                        setOtpSent(false);
                      }}
                      className="text-blue-700 hover:underline"
                    >
                      {phoneMode === 'password' ? 'Use OTP instead' : 'Use password instead'}
                    </button>
                    {phoneMode === 'otp' && (
                      <button
                        type="button"
